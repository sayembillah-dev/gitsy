...
// Commit graph renderer (Phase 4). Layout is pure (src/core/graphLayout.ts);
// this component only paints it. Nodes are spring-animated by framer-motion,
// keyed on structural hash, so a new commit pops in and lane changes glide.
...
export default function GraphSvg({ snapshot }: { snapshot: RepoSnapshot }) {
    ...
    <div className="relative font-mono" style={{ width: '100%', minWidth: graphW + 220, height: h }}>
      ...
      {layout.nodes.map((n) => (
        <motion.div
          ...
          animate={{ opacity: 1, scale: 1, x: x(n.lane) - R, y: y(n.row) - R }}
        ...
        >
          <div
            ...
            style={{
              width: R * 2,
              height: R * 2,
              borderColor: n.isHead ? 'var(--st-head)' : 'var(--rule)',
              backgroundColor: n.isHead ? 'var(--st-head)' : 'var(--panel)',
              boxShadow: n.isMerge ? '0 0 0 3px var(--ground), 0 0 0 4px var(--rule)' : undefined,
            }}
          />
          <div
          ...
          >
            ...
            <span className="pl-1 text-[11px] text-ink-dim">{n.message}</span>
          </div>
        </motion.div>
      ))}
    </div>
